...
def getDocsWithWord(word):
	...
	for line in index:
		line = line.split(" ", 1)
		if line[0]==word:
			doclist = (line[1].strip('\n'))
			doclist = doclist.split(' ',1)
			doclist = doclist[1].split(',')
			#print "Doclist: " + str(doclist[:-1])
			for word_instance in doclist[:-1]:
...
for query in queries:
	...
	print "Res: " + " ".join([str(i) for i in docs])